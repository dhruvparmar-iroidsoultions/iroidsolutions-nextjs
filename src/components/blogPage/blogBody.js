...
const BlogBody = ({ post, latestBlogs }) => {
  ...
  const [shareIcon, setShareIcon] = useState("/share-icon.svg");

  const copyLink = () => {
    navigator.clipboard.writeText(socialLink.url);
    setShareIcon("/copied-icon.svg");
    setTimeout(() => {
      setShareIcon("/share-icon.svg");
    }, 3000);
  };
          ...
          <div className="d-flex flex-wrap align-items-center justify-content-between fw-medium fs-5">
            ...
            <p className="mb-0 me-3 me-xl-5 d-flex align-items-center gap-2">
              <Image
                ...
                height={25}
              />
              <span>{post.blogdate}</span> {/* {formattedDate} */}
            </p>
            <p className="mb-0 me-3 me-xl-5 d-flex align-items-center gap-2">
              <Image
                src={"/blog-time.png"}
